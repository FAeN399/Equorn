 ...
 */
function formatPowers(entity: EntityConfig, powers: EntityConfig['powers']): string {
  if (!powers || !powers.length) return 'This entity has no defined powers.';
  
  return powers.map(power => `### ${power.name}

${power.description}

**Effects**:
${power.effects.map(effect => `- ${effect}`).join('\n')}
`).join('\n');
}
